...
public class GetHealingAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (method == null || data.size() == 1){
            ...
            locators = data.stream().flatMap(it-> it.getResults().stream()).collect(Collectors.toSet());
        } else {
        ...
        if(!locators.isEmpty()){
            new ResultPopup(e).createList("Healing Results", locators.toArray(new HealingResultDto[0]), (position, value) -> {
    ...
    private Set<HealingResultDto> findResults(PsiMethod method){
        return data.stream().filter(it-> it.getMethodName().equals(method.getName())).flatMap(it-> it.getResults().stream()).collect(Collectors.toSet());
    }
